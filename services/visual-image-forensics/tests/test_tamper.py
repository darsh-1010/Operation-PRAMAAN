...
class TestTamperDetector(unittest.TestCase):
    ...
    def test_checkpoint_contains_loss_function_and_confusion_matrix(self):
        """Verify that sidtd_efficientnet_b3.pth checkpoint contains loss_function and confusion_matrix."""
        ckpt_path = Path(__file__).resolve().parent.parent / "models" / "sidtd_efficientnet_b3.pth"
        if ckpt_path.exists():
            checkpoint = torch.load(ckpt_path, map_location="cpu")
            self.assertIn("loss_function", checkpoint)
            self.assertEqual(checkpoint["loss_function"]["name"], "CrossEntropyLoss")
            self.assertIn("confusion_matrix", checkpoint)
            ...
            self.assertIn("precision", cm)
            self.assertIn("recall", cm)
            self.assertIn("f1_score", cm)
