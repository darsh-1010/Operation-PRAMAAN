...
def main() -> None:
    ...
    # A valid submission (passport + selfie) must return the API_CONTRACT.md response shape.
    ...
    # A selfie that isn't a decodable image is accepted (assumed video) — but not empty.
    status, result = _post_screen({"selfie": True}, {"selfie": b"not-a-real-video-but-not-empty-either"})
    assert status == 200, result

    # A document photo that isn't a real image must be rejected, unlike the lenient selfie.
