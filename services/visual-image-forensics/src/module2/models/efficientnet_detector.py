        ...
        if self.checkpoint_path and self.checkpoint_path.exists():
            try:
                ...
                self.checkpoint_metadata = {
                    k: v for k, v in checkpoint.items() if k not in ("state_dict", "model_state_dict")
                } if isinstance(checkpoint, dict) else {}
                self.loss_function_info = self.checkpoint_metadata.get("loss_function")
                self.confusion_matrix_info = self.checkpoint_metadata.get("confusion_matrix")
                ...
                logger.info(
                    f"Loaded SIDTD checkpoint from {self.checkpoint_path} "
                    f"(loss_function: {self.loss_function_info.get('name') if self.loss_function_info else 'N/A'}, "
                    f"has_confusion_matrix: {self.confusion_matrix_info is not None})"
                )
