"""Process-pool isolation for the heavy native-library inference calls (PaddleOCR/OpenCV)
this service makes.

Why this exists: a native crash (segfault) inside a C++ library like PaddlePaddle can't be
caught by Python's try/except — it kills the whole OS process. Before this module, that meant
one bad image could take down an entire uvicorn worker (and every other request it was
serving) — confirmed directly: PaddleOCR segfaults on real image inference in this service's
Docker environment, reproduced with no threading involved at all.
...
_TASK_TIMEOUT_SECONDS = int(os.environ.get("INFERENCE_TIMEOUT_SECONDS", "120"))
